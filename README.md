...
> **模式建议：请使用默认的 `wysiwyg` 模式（推荐，日常观感最接近 Typora）；`ir`（即时渲染）仍在开发中、不稳定。**
> 区别在于 **Markdown 标记是否显示**：`wysiwyg` 完全隐藏标记（贴近 Typora 默认观感），`ir` 则在**所有块**里都保留标记（淡化显示）。
...
- **同标签实时排版（Typora 风格）**
  ...
  - 提供两种编辑形态（`typoraMd.mode` 配置，**默认 `wysiwyg`**）：
    - ✅ `wysiwyg`（**推荐**，默认，**最接近 Typora**）：标记**完全隐藏**，只看得到渲染结果（Typora 也只在光标所在那一块临时显示标记）；输入先作为文本保留，标记写完整（如 `> `）才转成结构，空行也不会被“吃掉”。
    - ⚠️ `ir`（即时渲染，**实验性 / 开发中，不稳定**）：**所有块**的标记都**保留可见**（淡化显示），边输入边解析结构。已知问题：行首输入 `>` 会立即变成引用、空行没有任何可见标记、回车产生的空行不会写回文件。**不建议日常使用。**
...
7. **插入表格**：点工具栏的表格按钮，在弹出的小网格里**滑动选择行列数**（最多 10 × 10），单击即插入。
   选 **N 行**就是表格共 N 行（**含表头行**，例如选 `3` 行 = 表头 + 2 行数据）；
   在已有文字的段落里插入时表格会自动另起一行，不会把原来的文字并进表头。
   表格的增删行列、对齐等操作在**单元格内右键**选。
8. **代码块**：点工具栏的代码块按钮会先让你**选语言**（一排常用语言快捷按钮，或直接输入
   `js` / `python` / `json` 等，留空 = 纯文本），确定后再插入；已有代码块可在上面**右键** →
   `编辑代码语言…` 随时改（改完高亮**立即更新**，无需重开文件）。同一菜单里还有「复制代码」「删除代码块」。
9. **跳出代码块 / 公式块**：在块内按 **`Cmd + Enter`**（Windows/Linux 上是 `Ctrl + Enter`，**与 Typora 一致**），
   跳到块后新建一个空行（光标就在这个新行里，可以直接接着写）；`Enter` 保持原有行为（块内新增一行）。
   快捷键可改：`typoraMd.exitBlockKeys`（如 `escape`、`shift+enter`、`ctrl+enter,escape`，留空 = 关闭）。
   作用范围由 `typoraMd.exitBlockScope` 控制（默认 `code,math`）；**引用块不受影响**（保留 Vditor 原生行为），
   如需一并启用可改成 `code,math,quote`。
10. **Enter = 新建段落**（与 Typora 一致）：在段落末尾回车，源码里新段落的**前后各留一个空行**（即空 2 行）；
    在**空行**上回车则会**再新建一个空行**（源码 3 行 → 5 行 → 7 行 …，可以一直按）。
    代码块 / 公式块 / 列表 / 引用 / 表格内的回车**不受影响**，仍走 Vditor 原生行为。
11. **空行会保留**：代码块 / 公式块后面的空行会**如实渲染成可点击的空行**，并且**不会被折叠或删掉**。
    详见下文「已知限制」里的空行规则。
...
| `typoraMd.mode`        | `wysiwyg` | 编辑形态：`wysiwyg`（标记隐藏，**推荐**，最接近 Typora）/ `ir`（标记始终可见，实验性不稳定） |
...
| `typoraMd.showBlockPanel` | `false` | 悬停块左侧标记时是否弹出块操作面板（上移 / 下移 / 删除 / 锚点 ID） |
| `typoraMd.exitBlockKeys` | `ctrl+enter` | 代码块 / 公式块内**跳出**的快捷键（macOS 上 `ctrl` 即 `Cmd`） |
| `typoraMd.exitBlockScope` | `code,math` | `exitBlockKeys` 对哪些块生效：可组合 `code` / `math` / `quote` |
...
- 两者的区别是 **Markdown 标记是否显示**：`wysiwyg` 完全隐藏标记（**最接近 Typora 的默认观感** —— Typora 也只在光标所在那一块临时显示标记），`ir` 则在所有块里都显示标记（淡化）。
...
Markdown 里空行只是「块分隔符」、本身不是元素，所以 Vditor 的引擎（Lute）每次打开文件都会把连续空行
合并成一个，原来的空行会被默默吃掉。本插件会在打开文件时把作者写的空行**还原成真正的空行段落**，保证：

- 代码块 / 公式块后面的空行**看得见、点得到**，光标能停上去；
- 保存时**不会**被折叠或删除，反复打开/保存也不会越写越多（已实测幂等）。

换算关系与 **Typora 一致**：编辑器里**新建一个空行 = 源码里多 3 行空行**
（新建的“空行”实际是一个段落，而每个段落在源码里前后各占一个空行）：
...
因此只要空行数是**奇数**（Typora 只会产生这种）就能**完全无损**地往返。
如果手工写出偶数行（2 行、4 行、6 行）—— Typora 不会生成这些值 —— 会**向下取到奇数**
（4 → 3、6 → 5），这是 Markdown 语法本身的限制。普通段落之间的空行目前仍按 Markdown 规范合并。
